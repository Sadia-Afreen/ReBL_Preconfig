...
def clean_xml(xml_string):
    # This regex removes most non-printable/invalid characters
    return re.sub(r'[^\x09\x0A\x0D\x20-\x7E]', '', xml_string)

def get_current_hierarchy(device):
    # 1. Get the raw hierarchy string
    raw_xml = device.dump_hierarchy()
    
    # 2. Clean the string: Remove characters that break XML parsers
    # This regex removes control characters except for tab, newline, and carriage return
    clean_xml = re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f]", "", raw_xml)
    
    try:
        # 3. Parse directly from the string instead of a file
        root = ET.fromstring(clean_xml)
        return ET.ElementTree(root)
    except ET.ParseError as e:
        print(f"Failed to parse XML: {e}")
        # Optional: Save the 'clean_xml' to a file only if it fails, for debugging
